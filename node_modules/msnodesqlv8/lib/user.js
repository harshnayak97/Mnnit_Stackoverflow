 ...
 */
  ...
  function SqlTypes () {
    ...
    function Table (typeName, cols) {
      ...
      var columns = []
      var schema = 'dbo'
      var unqualifiedTableName = typeName
      var schemaIndex = typeName.indexOf('.')
      if (schemaIndex > 0) {
      ...
      if (cols && Array.isArray(cols)) {
        cols.forEach(function (c) {
          columns.push(c)
          if (c.hasOwnProperty('schema_name')) {
    ...
    function TvpFromTable (p) {
      ...
      if (p.hasOwnProperty('columns') && p.hasOwnProperty('rows')) {
        var cols = p.columns
        var rows = p.rows
        var c
        tp.row_count = rows.length
        for (c = 0; c < cols.length; ++c) {
          var v = fromRow(rows, c)
          var ty = cols[c].type
    ...
    function getSqlTypeFromDeclaredType (dt, p) {
      switch (dt.declaration) {
        ...
        case 'varchar': case 'uniqueidentifier':
          return VarChar(p)
        ...
        case 'binary': case 'hierarchyid': case 'varbinary':
          return VarBinary(p)
